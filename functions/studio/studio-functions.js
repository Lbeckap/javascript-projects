...
function reverseCharacters(input) {
    ...
    if (typeof input === "number"){
        reversedInput = String(input).split('').reverse().join('');
...
function completeReversal(arr) {
    ...
    for (let i = 0; i < arr.length; i++) {
        reversedArr.unshift(reverseCharacters(arr[i]));
    }
    return reversedArr;
}
...
function funPhrase(str) {
    let snippet = '';
    if (str.length <= 3) {
        snippet += str.split('').pop();
    } else {
        snippet += str.split('').slice(0, 3).join('');
    }
    let newPhrase = `We put the '${snippet}' in '${str}'.`
